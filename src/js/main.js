...
window.changeLanguage = function(lang) {
  ...
  // Statik Metin Çevirileri (Menü ve Rozetler)
  if (lang === 'en') {
...
function renderPage(data) {
  ...
  if (data.reviews) {
    ...
    if (reviewsGrid && data.reviews.items) {
      ...
      data.reviews.items.forEach(review => {
        ...
        card.innerHTML = `
            <div class="flex items-center mb-4 space-x-1">${starsHtml}</div>
            <p class="text-text-muted italic mb-6 flex-grow">"${review.comment}"</p>
            <div class="flex items-center mt-auto">
                <div class="flex-shrink-0 w-10 h-10 rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold text-sm">${initials}</div>
                <div class="ml-3">
                    <p class="text-sm font-semibold text-text-main">${review.name}</p>
                    <div class="flex items-center text-xs text-text-muted"><span class="mr-1">Google Review</span></div>
                </div>
            </div>
        `;
  ...
  // 6. Hakkımızda (Hekim) Bölümü
  if (data.about) {
    updateElement('about-title', data.about.title);
    updateElement('about-name', data.about.name);
  ...
  // 9. Sosyal Medya İkonları (Google Scholar Eklendi)
  ...
  if (socialContainer && data.social) {
    ...
    const iconScholar = '<svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M12 3L1 9l11 6 9-4.91V17h2V9M5 13.18v4L12 21l7-3.82v-4L12 17l-7-3.82Z"/></svg>';

    // Link Kontrolleri ve Ekleme
    if (data.social.instagram) {
        socialContainer.innerHTML += `<a href="${data.social.instagram}" target="_blank" title="Instagram" class="text-text-muted hover:text-[#E1306C] transition-colors p-2 bg-white rounded-full shadow-sm hover:shadow-md border border-gray-100 flex items-center justify-center w-10 h-10">${iconInsta}</a>`;
    }
    if (data.social.facebook) {
        socialContainer.innerHTML += `<a href="${data.social.facebook}" target="_blank" title="Facebook" class="text-text-muted hover:text-[#1877F2] transition-colors p-2 bg-white rounded-full shadow-sm hover:shadow-md border border-gray-100 flex items-center justify-center w-10 h-10">${iconFb}</a>`;
    }
    if (data.social.googleMaps) {
        socialContainer.innerHTML += `<a href="${data.social.googleMaps}" target="_blank" title="Google Maps" class="text-text-muted hover:text-[#EA4335] transition-colors p-2 bg-white rounded-full shadow-sm hover:shadow-md border border-gray-100 flex items-center justify-center w-10 h-10">${iconMaps}</a>`;
    }
    // Scholar Kontrolü
    if (data.social.googleScholar) {
        socialContainer.innerHTML += `<a href="${data.social.googleScholar}" target="_blank" title="Google Scholar" class="text-text-muted hover:text-[#4285f4] transition-colors p-2 bg-white rounded-full shadow-sm hover:shadow-md border border-gray-100 flex items-center justify-center w-10 h-10">${iconScholar}</a>`;
    }
  }

  // 10. Footer ve Yüzen Butonlar
  updateElement('footer-brand', data.general.brandName);
  updateElement('current-year', new Date().getFullYear());
  addFloatingButtons(data.general.whatsappNumber, data.social.instagram, data.general.whatsappMessage);
}
...
function addFloatingButtons(waNumber, igLink, waMessage) {
  ...
  // Mesajı encode edip linke ekle
  const encodedMsg = waMessage ? `?text=${encodeURIComponent(waMessage)}` : '';
  const waLink = `https://wa.me/${waNumber}${encodedMsg}`;
